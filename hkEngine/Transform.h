...
public:
	...
	Transform TransposeTransform(const Transform& t) {
	...
	bool HasScale() const {
		float la2 = (*this)(float3(1, 0, 0)).LengthSquared();
		float lb2 = (*this)(float3(0, 1, 0)).LengthSquared();
		float lc2 = (*this)(float3(0, 0, 1)).LengthSquared();
...
#undef NOT_ONE
	...
	//TODO:  (SurfaceInteraction):  SurfaceInteraction operator()(const SurfaceInteraction& si) const;
	...
	inline Vector3<T> operator()(const Vector3<T>& v, const Vector3<T>& vError,
		Vector3<T>* vTransError) const;
	inline Ray operator()(const Ray& r, float3* oError,
		float3* dError) const;
	inline Ray operator()(const Ray& r, const float3& oErrorIn,
		const float3& dErrorIn, float3* oErrorOut,
		float3* dErrorOut) const;

	friend std::ostream& operator<<(std::ostream& os, const Transform& t) {
		os << "t=" << std::endl << t.m << ", inv=" << std::endl << t.mInv;
		return os;
	}
...
Transform LookAt(const float3& pos, const float3& look, const float3& up);
...
inline Ray Transform::operator()(const Ray& r, float3* oError,
	float3* dError) const {
	float3 o = (*this)(r.o, oError);
	float3 d = (*this)(r.d, dError);
	float tMax = r.tMax;
	float lengthSquared = d.LengthSquared();
	if (lengthSquared > 0) {
		float dt = Dot(Abs(d), *oError) / lengthSquared;
...
inline Ray Transform::operator()(const Ray& r, const float3& oErrorIn,
	const float3& dErrorIn, float3* oErrorOut,
	float3* dErrorOut) const {
	float3 o = (*this)(r.o, oErrorIn, oErrorOut);
	float3 d = (*this)(r.d, dErrorIn, dErrorOut);
	float tMax = r.tMax;
	float lengthSquared = d.LengthSquared();
	if (lengthSquared > 0) {
...
public:
	...
	static void Decompose(const Matrix4x4& m, float3* T, Quaternion* R,
		Matrix4x4* S);
	void Interpolate(float time, Transform* t) const;
	Ray operator()(const Ray& r) const;
	RayDifferential operator()(const RayDifferential& r) const;
	float3 operator()(float time, const float3& v) const;
	bool HasScale() const {
		return startTransform->HasScale() || endTransform->HasScale();
	}
	Bounds3f MotionBounds(const Bounds3f& b) const;
	Bounds3f BoundPointMotion(const float3& p) const;
